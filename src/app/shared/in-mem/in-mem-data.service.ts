...
export class InMemDataService implements InMemoryDbService {
  ...
  get(reqInfo: RequestInfo) {
    ...
    if(is(reqInfo, 'device')) {
        return ajax('assets/data/device.json').pipe(map((responese: any) => {
            return {headers, url, status: STATUS.ok, body: {device: responese.response.device}}
        }), switchMap(response  => reqInfo.utils.createResponse$(() => response)))
    }
